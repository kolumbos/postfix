...
RUN set -eux; \
    apt-get update; \
    apt-get install --yes --no-install-recommends ca-certificates git make m4 build-essential libdb5.3-dev libnsl-dev libicu-dev libssl-dev libtlsrpt0-dev; \
...
ARG CCARGS="-DMYORIGIN_FROM_FILE -DHAS_SSL -I/usr/include/openssl -DUSE_TLS -DUSE_TLSRPT"
ARG AUXLIBS="-lssl -lcrypto -lpthread -ltlsrpt"
...
ARG POSTDROP_GID=1001

RUN set -eux; \
    apt-get update; \
    apt-get install --yes --no-install-recommends ca-certificates libdb5.3 libnsl2 libicu76 gettext-base libssl3 libtlsrpt0; \
    rm -rf /var/lib/apt/lists/*
...
COPY --from=postfix-build /install /
